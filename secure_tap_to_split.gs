/**
 * TapToSplit: Secure Google Apps Script for Splitwise iOS Shortcut
 * HANDLES ALL ACTIONS VIA POST: Fetching groups, Fetching members, Creating expenses.
 */
...
// ==========================================
// 1. DEPRECATED: doGet Function (Security Update)
// ==========================================
function doGet(e) {
  return ContentService
    .createTextOutput(JSON.stringify({ error: "GET requests are disabled for security. Please use POST with your api_key in the request body." }))
    .setMimeType(ContentService.MimeType.JSON);
}

// ==========================================
// 2. MAIN: doPost Function (Handles fetching & creating)
// ==========================================
function doPost(e) {
  try {
    const requestData = JSON.parse(e.postData.contents);
    
    // Validate API key is provided securely in body
    ...
    const groupNameInput = requestData.Group || requestData.group_name;

    // ----------------------------------------------------
    // SCENARIO A: CREATE EXPENSE 
    // Triggered if an 'amount' is provided
    // ----------------------------------------------------
    if (amount) {
      const expenseParams = {
    ...
    // ----------------------------------------------------
    // SCENARIO B: FETCH MEMBERS (For specific group)
    // Triggered if a 'group_name' is provided, but NO amount
    // ----------------------------------------------------
    else if (groupNameInput) {
      const group = findGroupByName(groupNameInput, false, apiKey); 
    ...
    // ----------------------------------------------------
    // SCENARIO C: FETCH ALL GROUPS
    // Triggered if neither 'amount' nor 'group_name' is provided
    // ----------------------------------------------------
    else {
